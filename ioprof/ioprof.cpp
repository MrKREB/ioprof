...
IoProf::IoProf()
{
	hNtDll = ntdllapi.sm_LoadNTDLLFunctions();
}


IoProf::~IoProf()
{
	ntdllapi.sm_FreeNTDLLFunctions(hNtDll);
}
...
DWORD IoProf::findProcessId(const char *processname)
{
	HANDLE hProcessSnap;
	PROCESSENTRY32 pe32;
	DWORD result = NULL;
...
BOOL IoProf::sm_EnableTokenPrivilege(LPCTSTR pszPrivilege)
{
	HANDLE hToken = 0;
		...
		&tkp.Privileges[0].Luid))
	{
		...
		ntdllapi.gNtAdjustPrivilegesToken(hToken, FALSE, &tkp, 0,
...
std::vector<smPROCESSINFO> IoProf::sm_GetNtProcessesInfo()
{
	HANDLE hProcessSnap;
	PROCESSENTRY32 pe32;
	std::vector<smPROCESSINFO> ppi;
	smPROCESSINFO spi = { 0 };
	...
	do {
		if (!sm_GetNtProcessInfo(pe32.th32ProcessID, &spi))
			continue;
		ppi.push_back(spi);
	} while (Process32Next(hProcessSnap, &pe32));
...
BOOL IoProf::sm_GetNtProcessInfo(const DWORD dwPID, smPROCESSINFO *ppi)
{
